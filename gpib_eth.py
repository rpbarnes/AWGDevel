...
#{{{ the underlying gpib usb class, which has older instruments integrated (separate them later
class gpib_usb():
    def __init__(self,usbnumber,timeout = 5):
        # Switch for OS X
        self.flags = {}
        self.serial = serial.Serial(usbnumber,timeout=timeout)
    ...
    def oldread(self,addr = None,timeout = None):
        ...
        if timeout is not None:
            oldtimeout = self.serial.getTimeout()
...
def hp34401A(comport = 7):
    ip = '192.168.0.12'
    multiMeter = gpib(ip)
    multiMeter.setaddr(comport)
    multiMeter.respond('*IDN?')
    multiMeter.respond('READ?') # this returns the voltage read on the multimeter
...
class agilent():#{{{
    ...
    def readvoltage(self):
        ...
        return voltage
    def setvoltage(self,ch1V,ch2V = None,ch3V = None):
        if ch2V == None:
            ch2V = ch1V
            ch3V = ch1V
        self.command_send(':CHANnel1:SCALe %.2fV\n'%ch1V)
        self.command_send(':CHANnel2:SCALe %.2fV\n'%ch2V)
        self.command_send(':CHANnel3:SCALe %.2fV\n'%ch2V)
        return
    def clear(self):
    ...
    def waveform_complex(self, real_ch = 2, imag_ch = 3,avg_pts = 1):
        real_data = self.waveform(real_ch, avg_pts)
        imag_data = self.waveform(imag_ch, avg_pts)
        data_shape = ndshape([len(real_data.getaxis('t'))],['t'])
        data = data_shape.alloc(dtype = 'complex128')
        data.labels(['t'],[real_data.getaxis('t')])
        data.data = real_data.data + 1j*imag_data.data
        return data
    ...
    def waveform(self, channel, avg_pts = 1, preamble = None, get_preamble = False):
        ...
        if get_preamble:
            ...
            data_shape = ndshape([len(xdata)],['t'])
            ret_data = data_shape.alloc(dtype = 'float')
            ret_data.labels(['t'],[xdata])
            ret_data.data = ydata
            return ret_data
            #return nddata(ydata,[-1],['t'],axis_coords = [xdata]) # this probably works I'm just being an idiot
    ...
    def Waveform_auto(self):
        ...
        xdata, real_y = self.Waveform(2)
        xdata, imag_y = self.Waveform(3)
...
#{{{ Stanford lock-in
class lockin_amp ():
    def __init__(self,gpibaddress=6,sock = 1234,ip = '127.0.0.1'):
        self.g = gpib(ip = ip,sock = sock)
        self.gpibaddress = gpibaddress
        self.g.setaddr(self.gpibaddress)
        try:
...
class field_controller ():
    def __init__(self,gpibtalk=4,gpiblisten=36,sock = 1234,ip = '127.0.0.1',comport=5):
        # talk means device talks, computer listens; listen means device listens, computer talks
        if ip:
            self.g = gpib(ip = ip,sock = sock)
        else:
            self.g = gpib_usb(comport)
