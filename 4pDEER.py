...
scans = 10 # number of DEER traces summed together

bandpassFilter = 500e6 # filter for data
    

### Verify ELDOR delay will not exceed d2 ###
if (d2 - np.max(delayELDOR) - pELDOR) < 1.0:
    print '*'*50
    print('\tWARNING!!!\n\tdELDOR + pELDOR > d2\n\tautomatically correcting length of ELDOR trace')
    nptsELDOR = int(np.floor((d2 - pELDOR - dELDOR) / d30))
    delayELDOR = np.r_[dELDOR:(nptsELDOR-1)*d30+dELDOR:1j*nptsELDOR]
...
if doPhaseCycle:
    if phaseCycle == '8-step':
        ...
        ph1 = np.r_[0,np.pi]
        ph2 = np.r_[0]
        ph3 = np.r_[0,np.pi/2.,np.pi,3.*np.pi/2.]
        ph4 = np.r_[0]
...
agilentPosition = dx + 0.5*p0 + p1 + 2*d2 + d0# + 500e-9 + 20e-9#+ 280e-9 + 50e-9
...
print ('Time Elapsed: %0.2f seconds'%totalTime) 
